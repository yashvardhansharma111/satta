...
type ApiCell = { topDigits: [string,string,string]; main: string; bottomDigits: [string,string,string]; isRed: boolean };
type ApiRow  = { startDate: string; endDate: string; cells: ApiCell[] };
type WeekRow = { startLabel: string; endLabel: string; cells: ApiCell[] };
...
export default function LaxmiNightPanelPage() {
  ...
  const thStyle = (minW = 90): React.CSSProperties => ({
    backgroundColor: C.yellow, color: C.navy, fontWeight: 'bold', fontSize: '15px',
    padding: '10px 4px', textAlign: 'center', border: `1px solid ${C.red}`, minWidth: `${minW}px`,
  });
  ...
  return (
    <div style={{ backgroundColor: C.peach, minHeight: '100vh', fontFamily: 'Arial, sans-serif', paddingBottom: '20px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', margin: '6px 8px' }}>
        ...
        <Link href="/jodi/laxmi-night"><button style={{ backgroundColor: C.indigo, color: C.white, border: 'none', borderRadius: '8px', padding: '7px 14px', fontWeight: 'bold', cursor: 'pointer', fontSize: '13px' }}>Jodi Chart</button></Link>
      </div>
            ...
            <thead>
              <tr>
                <th colSpan={8} style={{ backgroundColor: C.indigo, color: C.white, fontWeight: 'bold', fontSize: '20px', padding: '12px 20px', textAlign: 'center', letterSpacing: '1px' }}>
                  LAXMI NIGHT PANEL CHART
                </th>
              </tr>
              <tr>
                <th style={thStyle(90)}>Date</th>
                {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((d) => <th key={d} style={thStyle(90)}>{d}</th>)}
              </tr>
            </thead>
            <tbody>
              {weeks.length === 0 ? (
                <tr><td colSpan={8} style={{ padding: '30px', textAlign: 'center', color: C.navy, fontWeight: 'bold', border: `1px solid ${C.red}` }}>No data available yet.</td></tr>
              ) : weeks.map((week, wi) => (
                      ...
                      <td key={di} style={{ border: `1px solid ${C.red}`, padding: '4px 2px', textAlign: 'center', minWidth: '90px', height: '68px', verticalAlign: 'middle' }}>
                        {jodi ? (
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {showTop && (
                              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', fontSize: '11px', fontWeight: 'bold', color: c.isRed ? C.red : '#000', lineHeight: 1.2, marginRight: '3px' }}>
                            ...
                            {showBottom && (
                              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', fontSize: '11px', fontWeight: 'bold', color: c.isRed ? C.red : '#000', lineHeight: 1.2, marginLeft: '3px' }}>
